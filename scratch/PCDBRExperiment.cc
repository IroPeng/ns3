...
NS_LOG_COMPONENT_DEFINE("PCDBRExperiment");

PCDBRExperiment::PCDBRExperiment()
	:
	m_sinkNum(10),
	m_nodeNum(100),
	m_bNodeNum(20),
	...
	m_simTime(1000),
	m_asciitracefile("PCDBRExperiment.asc")
{
}

void
PCDBRExperiment::PosConfigure()
{
...
void
PCDBRExperiment::NameNodes(NodeContainer &nodeContainer, string prefix)
{
	NS_LOG_FUNCTION(this);
...
void
PCDBRExperiment::InstallInternetStack()
{
	NS_LOG_FUNCTION(this);

	UanL3Helper uanL3Helper;
	uanL3Helper.SetRouting("ns3::UanRoutingPCDBR");
	uanL3Helper.Install(sinks);
	uanL3Helper.Install(nodes);
	uanL3Helper.Install(bNodes);
}
void
PCDBRExperiment::Start(Ptr<UanRoutingPCDBR> routing, uint32_t nodeId, uint32_t pktNum)
{
...
PCDBRExperiment::Run()
{
	...
	for(int i = 0; i < m_pktNum; i++){
		...
		Ptr<UanRoutingPCDBR> routing = bNodes.Get(nodeId)->GetObject<UanRoutingPCDBR>();
		Simulator::Schedule(Seconds(sendTime), &PCDBRExperiment::Start, this, routing, nodeId+m_sinkNum+m_nodeNum, m_pktNum);
	}
