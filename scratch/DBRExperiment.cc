...
NS_LOG_COMPONENT_DEFINE("DBRExperiment");

DBRExperiment::DBRExperiment()
	:
	m_sinkNum(10),
	m_nodeNum(100),
	m_bNodeNum(20),
	...
	m_simTime(1000),
	m_asciitracefile("DBRExperiment.asc")
{
}

void
DBRExperiment::PosConfigure()
{
...
void
DBRExperiment::NameNodes(NodeContainer &nodeContainer, string prefix)
{
	NS_LOG_FUNCTION(this);
...
void
DBRExperiment::InstallInternetStack()
{
	NS_LOG_FUNCTION(this);

	UanL3Helper uanL3Helper;
	uanL3Helper.SetRouting("ns3::UanRoutingDBR");
	uanL3Helper.Install(sinks);
	uanL3Helper.Install(nodes);
	uanL3Helper.Install(bNodes);
}
void
DBRExperiment::Start(Ptr<UanRoutingDBR> routing, uint32_t pktNum)
{
	routing->SendDataAsSrc(pktNum);
}

void DBRExperiment::Run()
{
	NS_LOG_FUNCTION(this);
	...
	for(int i = 0; i < m_pktNum; i++){
		...
		Ptr<UanRoutingDBR> routing = bNodes.Get(nodeId)->GetObject<UanRoutingDBR>();
		Simulator::Schedule(Seconds(sendTime), &DBRExperiment::Start, this, routing, m_pktNum);
	}
...
{
	LogComponentEnable ("DBRExperiment",LOG_LEVEL_INFO);
	LogComponentEnable ("UanRoutingDBR",LOG_LEVEL_INFO);
