...
{
  NS_LOG_FUNCTION (this);
NS_LOG_UNCOND("Test enter uanphy::UpdatePowerConsumption");
  if (!m_energyCallback.IsNull ())
    {
	  NS_LOG_UNCOND("Test enter uanphy::UpdatePowerConsumption enter energycallback");
      m_energyCallback (state);
	  NS_LOG_UNCOND("Test enter uanphy::UpdatePowerConsumption ready to leave energycallback");
    ...
    case IDLE:
      {
        ...
        //NS_LOG_UNCOND(newsinr);
        //NS_LOG_UNCOND(m_rxThreshDb);
        if (newsinr > m_rxThreshDb)
          {
        	//NS_LOG_UNCOND("newsinr > m_rxThreshDb");
            m_state = RX;
            ...
            m_pktRxPdp = pdp;
            double txdelay = pkt->GetSize () * 8.0 / txMode.GetDataRateBps ();
            NS_LOG_UNCOND("Here is uan-phy-gen start recv packet check the  former event m_rxEndEvent id TS "<<m_rxEndEvent.GetTs()
            		<<" context "<<m_rxEndEvent.GetContext()<<" uid "<<m_rxEndEvent.GetUid());
            m_rxEndEvent = Simulator::Schedule (Seconds (txdelay), &UanPhyGen::RxEndEvent, this, pkt, rxPowerDb, txMode);
            NS_LOG_UNCOND("Here is uan-phy-gen start recv packet check the txdelay "<<txdelay<<" check the event id TS "<<m_rxEndEvent.GetTs()
            		<<" context "<<m_rxEndEvent.GetContext()<<" uid "<<m_rxEndEvent.GetUid());

            NS_LOG_UNCOND("Here is check the event");
...
UanPhyGen::RxEndEvent (Ptr<Packet> pkt, double rxPowerDb, UanTxMode txMode)
{
  ...
  NS_LOG_UNCOND("Enter uanphygen::rxendevent");
  if (m_pg->GetValue (0, 1) > m_per->CalcPer (m_pktRx, m_minRxSinrDb, txMode))
    {
      ...
      if (!m_recOkCb.IsNull ())
        {
    	  NS_LOG_UNCOND("call back recok connect");
