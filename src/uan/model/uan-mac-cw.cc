...
{NS_LOG_UNCOND("state is "<<m_state);
  if (m_state == RUNNING)
    {
...
UanMacCw::NotifyTxStart (Time duration)
{
  ...
  NS_LOG_UNCOND("Here is uanmaccw notifytxstart, check isrunning and canel");
...
{
	NS_LOG_UNCOND("Enter uanmaccw::phyrxpacketgood");
  UanHeaderCommon header;
  bool rmheadercommon = packet->RemoveHeader (header);
 if(rmheadercommon)
 {
	  NS_LOG_UNCOND("remove common packet successed dest address "<< header.GetDest ().GetAsInt ());
	  NS_LOG_UNCOND("Header contain "<<header);
 }
  if (header.GetDest () == m_address || header.GetDest () == UanAddress::GetBroadcast ())
    {
	  NS_LOG_UNCOND("Forward packet up cb envoke");
